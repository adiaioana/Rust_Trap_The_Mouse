...
use boardserver::{Board,gameboard_state};
use prereqserver::{BasicMessages,str_to_int};
#[derive(Clone)]
struct Room {
    room_nb: i32,
    player1: i32,
...
impl Room{
    pub fn new() -> Self {
        Room{room_nb:0, player1:0,player2:0,game_state:0, game_board:Board::new(), turn:1}
    }
}
...
fn room_handling_for_client( buffer:[u8;1024], state: SharedState, idclient:i32) -> String{ 
    ...
    process_room_request(arg1, arg2, &state, idclient)
}
...
fn handle_client(mut stream: TcpStream, state: SharedState, idclient:i32) {
    ...
    loop{
        ...
        if mess.contains("Succesfully joined:") {
            ...
            for word in mess.split_whitespace() {
                ind+=1;
                if ind>2 && !word.is_empty() { //"Succesfully joined room:"
    ...
    if gs==0 {
        ...
        if client_option=='W' {
            let begin=match SystemTime::now().duration_since(UNIX_EPOCH) {
                Ok(n) => n.as_secs(),
        ...
        match ifwinner {
            ...
            _ =>{
                if turn_of(&whichroom, &state)==type_of_pawn {
                    ...
                    let mut game_board=get_gameboard(&whichroom, &state);
                    //update moves string 
                    ...
                    for word in str::from_utf8(&buffer).unwrap().split_whitespace() {
                        ...
                        let number=str_to_int(word);
                        if number!=-1 {
                            var.push(number);
                        }
                    }
                    let has_moved:bool= if type_of_pawn ==2{
                    ...
                    //has_moved=true;
                    push_gameboard(&whichroom, &state, &game_board, has_moved);
                    // switch_turn(&whichroom, &state);
                    stream.write_all(game_board.clone().translate_to_moves(turns[type_of_pawn as usize], !has_moved).as_bytes()).expect("Failed to write to stream");
...
fn main() {
    ...
    let listener = TcpListener::bind("127.0.0.1:7881").expect("Failed to bind to address");
    ...
    for stream in listener.incoming() {
        ...
        fn push_gameboard(room_name: &str, state: &SharedState, a:&Board, changeturn:bool) {
            let mut rooms = state.lock().unwrap();
            let mut r:Room=Room::new();
            let mut tn=0; 
            for it in rooms.iter() {
...
*/
        ...
        fn parse_message(msg: &str) -> (String, String) { // verified
            ...
            if msg.contains("create") {
                ...
                for word in msg.split_whitespace(){
                    ...
                    if !word.contains("create") && !word.is_empty() {
                        whichroom=word.to_string();
                        break;
                    }
                }
            ...
            (String::from("invalid"), String::from("-"))
        ...
        match command {
            "create" => {
                ...
                let lg=rooms.len();
                let a: Room=Room{room_nb:lg as i32,player1:0,player2:0,game_state:0, game_board:Board::new(), turn:1};
                rooms.insert(command_room_name.to_string(),a);
                let ans=String::from("Succesfully created room: ");
                ans+command_room_name
            }
            "join" => {
                ...
                if let false=ok {
                    String::from("Unknown room, not able to join. Please [create]/[join room_name]/[print rooms] room.")
                }
                else {
            ...
            "print" => {
                ...
                String::from("Printed all rooms.\n")
            }
            _ => {
                String::from("Unknown command, please [create]/[join room_name] room.\n")
            }
        }
